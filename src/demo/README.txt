...
This is the README for a Java implementation of the Advanced Encryption Standard for a course in Cryptography and Network 
Security. As such, the README assumes that the user has partial technical knowledge in order to run the program. For more 
information on AES, view and research the provided keywords on the following section if needed. 

The project consisted on four AES implementations, one where values are calculated on execution time,  a second
where the values are on previously generated lookup tables, an EBC operating mode, and a CBC mode.

The user interface should facilitate operation and comes with this updated README to guide the user. The interface for the 
demonstration was implemented after the course was finished due to time constraints so, from a programmer's view, there
are various ways to utilize and run the given code. 
...
Cipher-block Chaining		Initialization Vector
Ciphertext			Key
...
For the other configurations, the file is expected to have the key in the first line of text followed by the input text, to be used as
plaintext or ciphertext depending if encrypted or decrypted, in the subsequent lines.

==================================================================================================
        CONTACT INFORMATION
==================================================================================================

For any questions, contact me through twitter @janoxepr.
