...
The project consisted on four AES implementations, one where values are calculated on execution time,  a second where the values 
are on previously generated lookup tables, an EBC operating mode, and a CBC mode.
...
demonstration was implemented after the course was finished due to time constraints so, from a programmer's view, there are 
various ways to utilize and run the given code. 
